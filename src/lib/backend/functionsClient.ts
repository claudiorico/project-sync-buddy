 ...
 *
 * Routes backend function calls to Supabase Edge Functions by default,
 * or to an external HTTP endpoint configured at deploy time.
 *
 * Env vars (optional):
 * - VITE_FUNCTIONS_BASE_URL: string
 *     Example: https://<project>.supabase.co/functions/v1
...
export async function invokeBackendFunction<TData = any>(
  ...
  // Default path: use Supabase Edge Functions via SDK
  if (!baseUrl) {
